...
use record_event::workflow::{InquirePrompter, WorkflowError, run_entry};

const SERVER_URL: &str = "http://127.0.0.1:9123";

fn main() {
    if std::env::args().len() != 1 {
        eprintln!("用法: record-event");
        std::process::exit(2);
    }
    if !std::io::stdin().is_terminal() {
        eprintln!("record-event 需要在交互式终端中运行");
        std::process::exit(2);
    }
    ...
    let exit_code = match run_entry(&api, &mut InquirePrompter) {
        Ok(code) => code,
